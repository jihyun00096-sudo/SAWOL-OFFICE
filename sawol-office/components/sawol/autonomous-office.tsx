...
export function AutonomousOffice({
  ...
  const loadStatus = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/office/tasks/${task.id}/autopilot/status`,
        {
          cache: "no-store",
        },
      );
    ...
    async (forceRetry = false) => {
      ...
      try {
        ...
        if (!hasWorkflow && plan.mode === "COLLAB") {
          if (
            plan.steps.some(
              (step) => !step.employeeId,
            )
          ) {
            throw new Error(
              "자동 협업에 필요한 AI 직원을 충분히 배정하지 못했습니다.",
            );
          }

          const workflowPayload = plan.steps.map(
            (step) => ({
              key: step.key,
              title: step.title,
        ...
        const response = await fetch(
          `/api/office/tasks/${task.id}/autopilot/start`,
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    ...
    // PAUSED는 자동 재조사를 이미 끝낸 뒤 대표 판단이 필요한 상태이므로
    // 페이지 새로고침 때 무한 재시작하지 않습니다.
    const shouldAutoStart =
      !job ||
      job.status === "QUEUED" ||
      (job.status === "RUNNING" && job.stale);
    ...
    <section className="rounded-[18px] border border-[#DDE4FA] bg-[#FBFCFF] p-4 sm:p-5 lg:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {paused ? (
              <span className="rounded-full bg-[#FFF5DD] px-2 py-1 text-[8px] font-semibold text-[#8A6824]">
                대표 판단 필요
              </span>
            ) : !failed && !closed ? (
          ...
          <p className="mt-1 max-w-[780px] text-[10px] leading-5 text-[#7A8290]">
            AUTO 업무는 검색 결과가 부족해도 즉시 멈추지 않고
            검색어를 재구성해 여러 차례 자동 재조사합니다.
            대표가 지정한 출처·수량·형식은 임의로 완화하지 않으며,
            자동 복구로 해결할 수 없는 경우에만 대표 판단을 요청합니다.
          </p>
        </div>
      ...
      {!hasWorkflow && !workflow ? (
        <div className="mt-4 rounded-[13px] bg-white p-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full bg-[#F3F5F8] px-2.5 py-1 text-[9px] font-semibold">
              복잡도 {plan.complexity}%
            </span>
            ...
            <span
              className={`rounded-full px-2.5 py-1 text-[9px] font-semibold ${
                plan.mode === "COLLAB"
                  ...
                  : "bg-[#ECF8F0] text-[#2D7650]"
              }`}
            >
              {plan.mode === "COLLAB"
                ? `협업 ${plan.steps.length}단계`
                : "단일 직원 처리"}
            </span>

            <span className="rounded-full bg-[#F7F8FA] px-2.5 py-1 text-[9px] text-[#7A818D]">
              협업 기준 60점
            </span>
          </div>

          <p className="mt-2 text-[10px] leading-5 text-[#7B828E]">
            {plan.reason}
          </p>
        </div>
      ) : null}
      ...
      <div className="mt-4 rounded-[13px] bg-white p-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="shrink-0 text-[18px] font-semibold text-[#242A33]">
            {progress}%
          </p>
        </div>
        ...
        <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#E9EDF7]">
          <div
            className="h-full rounded-full bg-[#3157D5] transition-all duration-500"
            style={{
              width: `${progress}%`,
            }}
          />
      ...
      {workflow?.steps?.length ? (
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {workflow.steps.map((step) => (
            <div
              key={step.id}
              className="rounded-[11px] border border-[#E9ECF3] bg-white px-3 py-2.5"
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="shrink-0 text-[8px] text-[#858C97]">
                  {stepStatusLabel[step.status] ??
                    step.status}
                </span>
              </div>

              <p className="mt-1 text-[8px] text-[#9A9FA8]">
                {step.employee ?? "미배정"}
              </p>
            </div>
          ))}
      ...
      {paused ? (
        <div className="mt-3 rounded-[11px] border border-[#F0DCA9] bg-[#FFF9E9] px-3.5 py-3">
          <p className="text-[10px] font-semibold text-[#7C5E1B]">
            AI 직원들이 자동 재조사를 모두 마쳤지만 조건을 임의로 바꾸지 않고 멈췄습니다.
          </p>

          <p className="mt-1.5 whitespace-pre-wrap text-[10px] leading-5 text-[#75694C]">
            {job?.last_message ||
              "대표가 지정한 출처·수량 조건을 모두 충족하지 못했습니다."}
          </p>

          <p className="mt-2 text-[9px] leading-4 text-[#9A895F]">
            다음 STEP에서 ‘현재 확보된 자료로 진행 / 기간 확대 / 출처 확대’처럼
            대표가 선택만 하면 자동 재개하는 판단 버튼을 연결할 수 있습니다.
          </p>
        </div>
      ) : failed ? (
        <p className="mt-3 rounded-[10px] bg-[#FFF2F2] px-3 py-2.5 text-[10px] leading-5 text-[#A64242]">
          {localError ||
            job?.last_error ||
            "기술 오류로 자동 실행이 중단되었습니다. 기존 결과와 협업 단계는 보존되어 있습니다."}
        </p>
      ) : job?.last_message ? (
        <p className="mt-3 rounded-[10px] bg-[#F1F6FF] px-3 py-2.5 text-[10px] leading-5 text-[#3157D5]">
          {job.last_message}
        </p>
      ) : null}
    </section>
